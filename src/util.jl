...
function sat(cone_roots_grammed,root_grammed, strict, lax)
    ...
    if p.status == MathOptInterface.INFEASIBLE && lax
        return true
    elseif p.status == MathOptInterface.OPTIMAL && strict
        #println(p.optval)
        return true
    else
        return false
...
function is_necessary_halfspace(gram,cone_roots,root) 
    ...
    for i in 1:10
        ...
        cone_roots_grammed = Vector{Vector{BigFloat}}([[approx(r,LP_PRECISION) for r in gram*cone_root] for cone_root in cone_roots])    
        root_grammed = Vector{BigFloat}([approx(r,LP_PRECISION) for r in gram*root])
